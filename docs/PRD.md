...
- D2 License. Chosen: Apache-2.0 for the core (consistent with Sextant). The LICENSE file is committed and pyproject.toml declares it. Constraint: Dissect is AGPL-3.0, so keep the optional Raw-mode path isolated, or be prepared to license that path AGPL, when Raw mode begins. Confirmed at Phase 8 (Raw mode start): isolation, not AGPL relicensing of the core. Dissect is declared only under the `raw` optional-dependency extra in pyproject.toml, never a core dependency, so a default install pulls in no AGPL code. All Dissect-dependent code lives in the single subpackage `casebound/ingest/raw`, which imports Dissect lazily (inside `read`, never at module load), so importing the package loads no AGPL code. No module outside that subpackage imports Dissect or the subpackage, so the entire core pipeline runs with zero AGPL code on its import graph. The normalization of the records the raw adapters emit lives in the Apache-2.0 core (`casebound/normalize/mappers/dissect.py`) and is fully golden-tested offline; only the thin Dissect byte-parsing is gated. The boundary is enforced in code by `tests/test_license_boundary.py` (no core import of `dissect` or `casebound.ingest.raw`; Dissect optional in pyproject; core license stays Apache-2.0). Installing the `raw` extra and using raw mode assembles a combined work that includes AGPL-3.0 code and is then subject to its terms; Casebound neither bundles nor redistributes Dissect. See `docs/raw-mode.md`.
