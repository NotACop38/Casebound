...
- D2 License. Chosen: Apache-2.0 for the core (consistent with Sextant). The LICENSE file is committed and pyproject.toml declares it. Constraint: Dissect is AGPL-3.0, so keep the optional Raw-mode path isolated, or be prepared to license that path AGPL, when Raw mode begins.
